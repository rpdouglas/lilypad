...
|--------------------|----------------|---------------------------|-----------------|--------------------------------------|
| Hero headline      | `font-display` | `text-5xl` / `text-6xl`   | `font-light`    | Homepage hero only                   |
| Section title      | `font-display` | `text-3xl` / `text-4xl`   | `font-normal`   | Section headings                     |
...
| Overline label     | `font-mono`    | `text-[10px]`             | `font-normal`   | Section numbers, badges              |
...
- Italic accent words in headings use coral: <em className="italic text-coral">word</em>
- Line height for headings: leading-tight (1.15). Body copy: leading-relaxed (1.75).
- Letter spacing for mono labels: tracking-[0.15em] to tracking-[0.2em].
...
| Pill    | 9999px | `rounded-full`  | Avatar circles, pill tags         |
...
| Modal      | `shadow-lg`    | Modals, drawers              |
| Focus ring | `shadow-coral` | Keyboard focus on interactive elements |
...
| Intent               | Duration | Use case                                  |
|----------------------|----------|-------------------------------------------|
| Hover / color change | 150ms    | Button hover, link color, border reveals  |
| Dropdown / accordion | 200ms    | Nav menu, FAQ expand                      |
| Page element reveal  | 500ms    | Framer Motion entrance animations         |
| Stagger delay step   | +80ms    | Each item in a staggered list             |
...
| md   | 768px  | `md:`           | Tablets                                  |
| lg   | 1024px | `lg:`           | Laptops — most desktop layout switches   |
| xl   | 1280px | `xl:`           | Wide desktop                             |

Always mobile-first. Base styles = mobile. Use lg: for desktop layout changes.
Minimum design target: 375px. Primary test sizes: 390px and 1280px.
...
1. Colors — use only named Tailwind classes (bg-forest, text-coral). No raw hex codes.
2. Fonts — headings always font-display. Body always font-sans. Labels always font-mono.
3. Spacing — use the spacing values from the table above. No arbitrary values like p-[22px].
4. Italic accent — coral italic word in headings is the brand signature. Use it on section titles.
