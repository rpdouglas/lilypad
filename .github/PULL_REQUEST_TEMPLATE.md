...
Every PR that touches UI or content must be reviewed against all three personas.
Check each box only when you are genuinely confident the answer is yes.
...
- [ ] If this touches the Services or About page — does it reinforce expertise and process clarity?

**Dana — The Active Client** *(portal user, mobile + desktop, needs clarity and no surprises)*
...
- [ ] Tailwind classes only — no inline styles, no CSS Modules, no arbitrary hex values
- [ ] Firebase calls are in `/lib/firebase/` only — not inside components directly
- [ ] New page components include a persona annotation comment at the top of the file
- [ ] No `console.log` statements left in the code
- [ ] Runs without errors locally (`npm run dev`)
- [ ] Lint passes (`npm run lint`)
- [ ] Tests pass (`npm run test`)
- [ ] Tested on mobile (375px) and desktop (1280px) via the Firebase preview URL

## Documentation Checklist

- [ ] If a new route was added — `docs/SITEMAP.md` is updated
- [ ] If a new dependency was added — an ADR exists in `docs/adr/`
- [ ] If architecture changed — `CLAUDE.md` is updated if conventions changed

---

## Preview URL

<!-- Paste the Firebase preview channel URL posted by the GitHub Actions bot below. -->
