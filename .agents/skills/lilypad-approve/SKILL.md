---
name: lilypad-approve
description: >-
  Runs the Execution Protocol for Lily Pad features. Activate this skill when the user
  approves a plan strategy (e.g. Strategy A, B, or C) to locked-in scope, verify files,
  and surgically write the codebase implementations.
---

# Execution Protocol (Plan Approval & Scoping)
...
> "Strategy [A/B/C] approved: [one sentence]. Reading plan file now."

### Gate 2 — File Verification (The Read Gate)

For every file you intend to create or modify, read its current contents now using the Read/View file tool. Do not rely on prior session memory or a previous conversation's context.
...
> **Ready for QA.** Run the testing skill to verify. Suggested smoke tests: [list 2–3 specific tests relevant to this feature and its primary persona].
> Run ticket-close after QA passes.

---

*Lily Pad Strategy & Design · Adapted from docs/prompts/APPROVAL.md*
